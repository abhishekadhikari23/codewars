...
would become E, Y would become B and so on. The Vigenère cipher consists of
...
Any character not in the alphabet must be left as is. For example (following
from above):

c.encode('CODEWARS'); // returns 'CODEWARS'
NOTE FOR PYTHON

The Python implementation requires Unicode conversion, i.e.
input.decode('utf-8'), output.encode('utf-8')

Trying to print utf-8 strings without encoding them first (as mentioned above)
...
class VigenereCipher(object):
    ...
    def __init__(self, key, alphabet):
        pass
        key = key.upper()
        for i in range(0, int(len(alphabet)/len(key)+2)):
            key = key + key
        ...
        for i in range(0,len(alphabet)):
            if(alphabet[i].isalpha()):
                c = ord(alphabet[i]) + ord(key[i])
                if(c>155):
                    c = c - 91
                    dic1.update({alphabet[i]:chr(c)})
                else:
                    c = c - 65
                    dic1.update({alphabet[i]:chr(c)})
        self.dic = dic1
    ...
    def encode(self, text):
        ...
        if(text[0].isupper()):
            for i in range(0,len(text)):
                if(text[i].isalpha()):
                    s = s + dic2[text[i]].upper()
                else:
                    s = s + text[i]
        else:
            for i in range(0,len(text)):
                if(text[i].isalpha()):
                    s = s + dic2[text[i].upper()].lower()
                else:
                    s = s + text[i]
        print(s)
        return s
